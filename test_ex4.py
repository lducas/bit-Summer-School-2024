...
from sol4 import enumerate, simple_enumeration
...
for dim in dimensions:
    for _ in range(instances_per_dim):
        bases.append(np.random.randint(-10, 10, size=(dim, dim)))
        targets.append(np.random.uniform(-10, 10, size=(dim,)))
        diameters.append(np.random.randint(4, 5))
...
def test_simple_enumeration_close_to_target(B, t, l):
    ...
    # x_reduced = np.linalg.solve(B, x - t)
    ...
    # y_reduced = np.linalg.solve(B, y - t)
