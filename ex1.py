...
def nearest_plane(B, Bs, t):
	...
	for i in reversed(range(n)):
		k = int(np.round(tt.dot(Bs[i]) / Bs[i].dot(Bs[i])))
		tt -= k * B[i]
		v += k * B[i]
...
def compare_norm_distrib(B, num_samples):
	...
	n, _ = B.shape
	data_SR = [norm((np.random.rand(n) - .5).dot(B)) for x in range(num_samples)]
	data_NP = [norm((np.random.rand(n) - .5).dot(Bs)) for x in range(num_samples)]
